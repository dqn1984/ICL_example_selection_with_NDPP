...
class IBNDataCollatorWithPadding:
    ...
    def __call__(self, samples: List[InputSample]) -> BatchEncoding:
        ...
        candidates = []
        eid2idx = {}
        V, B, C = load_ndpp_kernel(self.dataset, ondpp=False)
        ...
        for sample in samples:
            ...
            #print("!!!!!!!!!!!!", len(samples), len(sample.question_ids), len(sample.ctxs_candidates), len(sample.ctxs_candidates[0]))
                ...
                for eid in ctxs:
                    if eid in eid2idx:
                        ...
                        if eid < V.shape[0]:
                            ...
                            candidates.append(V.shape[0]-1)      
                ctx_indices.append(indices)
                #print("ctxs", ctxs, "indices", indices)
        ...
        V = V[candidates, :]
        B = B[candidates, :]
        kernel_matrix = torch.matmul(V, V.T) + torch.matmul(torch.matmul(B, C), B.T)
