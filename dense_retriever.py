...
from src.utils.ndpp_map import greedy_map_ndpp
from src.utils.kndpp import k_ndpp_sampling
from src.utils.mcmc_utils import load_ndpp_kernel
...
class DenseRetriever:
    ...
    def find(self):
        ...
        if self.dpp_search:
            ...
            func = partial(dpp, task_name=self.task_name, num_candidates=self.num_candidates, num_ice=self.num_ice,
...
def get_kernel(embed, candidates, scale_factor):
    ...
    #logger.info(f"sim_matrix: {sim_matrix.shape}")
    kernel_matrix = rel_scores[None] * sim_matrix * rel_scores[:, None]
    #logger.info(f"rel_scores: {rel_scores[None].shape}")
...
def get_ndpp_kernel(dataset, embed, candidates, scale_factor):
    ...
    # to balance relevance and diversity
    rel_scores = np.exp(rel_scores / (2 * scale_factor))
    # print(V.shape, B.shape, C.shape, kernel_matrix.shape, near_reps.shape, rel_scores.shape)
...
def dpp(entry, task_name='mrpc', num_candidates=1, num_ice=1, mode="map", dpp_topk=100, scale_factor=0.1):
    candidates = knn(entry, num_ice=dpp_topk)['ctxs']
    # logger.info(f"knn_cand: {candidates}")
    embed = np.expand_dims(entry['embed'], axis=0)
    #near_reps, rel_scores, kernel_matrix = get_kernel(embed, candidates, scale_factor)
    #logger.info(f"near_reps:{near_reps}; rel_scores:{rel_scores};kernel_matrix:{kernel_matrix};")
    ...
    if mode == "cand_random" or np.isinf(kernel_matrix).any() or np.isnan(kernel_matrix).any():
        ...
        W = torch.block_diag(torch.eye(V.shape[1]), C)
        ...
        #                                     pre_results=ctxs_candidates_idx)
        #ctxs_candidates_idx = k_ndpp_sampling(X, W, k=num_ice, random_state=1,
        #                                      ondpp=False, min_num_leaf=8, num_samples=num_candidates,
        #                                      pre_results=ctxs_candidates_idx)
        #ctxs_candidates_idx = k_ndpp_sampling(X, W, k=num_ice, random_state=1,
        #                                      ondpp=False, min_num_leaf=8, num_samples=num_candidates)
    else:
        ...
        # logger.info(f"line_num of input_B: {B.shape[0]}")
        # logger.info(f"row_num of input_B: {B.shape[1]}")
        #B = B.numpy()
        #B = rel_scores[:, None] * B 
        #B = torch.as_tensor(B)
